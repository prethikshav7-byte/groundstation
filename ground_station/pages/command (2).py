...
class CommandPage(QWidget):
    ...
    def _send_guarded(self, command: str) -> None:
        ...
        if spec is None:
            # Calibration commands are generated per sensor and are not
            # in the §10.5 table individually; they carry the same guard.
            spec = COMMANDS["REFRESH_PROCESSOR"] if base.startswith("CALIBRATE") \
                else None
        if spec is None:
            return
    ...
    def _send_move(self, actuator_id: str, delta: float) -> None:
        ...
        target = (actuator.position or actuator.spec.home) + delta
